...
fn spawn_app() -> String {
    ...
    let common_data = Arc::new(Mutex::new(common_data));

    // clone some values to pass to the thread that is going to read the thermometer in a thread

    let listener = TcpListener::bind("127.0.0.1:0").expect("Failed to bind random port");
    let port = listener.local_addr().unwrap().port();
    let server = thermostat_pi::run(listener, &common_data).expect("Failed to bind address");
